...
export default function RegisterPage() {
  ...
  const [form, setForm] = useState({ fullName: "", email: "", password: "", confirmPassword: "", role: "reader" });
  ...
  const getRoleForDB = (r) => (r === "reader" ? "Reader" : "Writer");
 ...
 const handleEmailSubmit = async (e) => {
  ...
  const eyeBtn = (show, toggle) => (
    <button type="button" onClick={toggle} className="absolute right-2.5 text-[#334155] hover:text-[#94A3B8]">
      {show ? <EyeOff size={14} /> : <Eye size={14} />}
    </button>
  );

  return (
    <div className="min-h-screen flex items-center justify-center p-5" style={{ background: "#07070E", position: "relative", overflow: "hidden" }}>
      ...
      <div className="reg-grid relative z-10 w-full max-w-[860px] overflow-hidden rounded-[20px]" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", border: "0.5px solid rgba(244,196,48,0.12)" }}>

        {/* Left Panel */}
        <div className="reg-left flex flex-col justify-between p-10 relative" style={{ background: "linear-gradient(145deg,#0E0D1F 0%,#12101E 60%,#0A0918 100%)" }}>
          <div className="flex items-center gap-2">
            ...
            <span style={{ ...FD, fontSize: 17, fontWeight: 700, color: "#F0EDE6" }}>Luminary</span>
          </div>

          <div className="flex flex-col items-center text-center">
            <BookStack />
            <h2 style={{ ...FD, fontSize: 20, fontWeight: 700, color: "#F0EDE6" }}>Read. Write.<br /><em style={{ fontStyle: "italic", color: ACC }}>Illuminate.</em></h2>
          </div>

          <p className="text-center text-[11px]" style={{ color: "#1E293B", ...F }}>
            Already have an account? <Link href="/auth/login" style={{ color: ACC }}>Sign in</Link>
          </p>
        </div>

        {/* Right Panel */}
        <div className="flex flex-col justify-center p-9" style={{ background: "#0C0B1A" }}>
          <h1 style={{ ...FD, fontSize: 22, fontWeight: 700, color: "#EDE9E0" }}>Create your account</h1>
          <p className="text-[12px] mb-6" style={{ color: "#334155", ...F }}>Free to join. Start reading or publishing today.</p>

          {apiError && <div className="mb-4 p-3 rounded-[9px] text-red-400 bg-red-500/10 border border-red-500/30 text-sm">{apiError}</div>}

          <form onSubmit={handleEmailSubmit} noValidate>
            <Field label="Full name" id="fullName" value={form.fullName} onChange={setField("fullName")} placeholder="Rafi Ahmed" error={errors.fullName} icon={User} />
            <Field label="Email address" id="email" type="email" value={form.email} onChange={setField("email")} placeholder="you@example.com" error={errors.email} icon={Mail} />

            <Field label="Password" id="password" type={showPw ? "text" : "password"} value={form.password} onChange={setField("password")} placeholder="Min. 6 characters" error={errors.password} icon={Lock} rightSlot={eyeBtn(showPw, () => setShowPw(p => !p))} />

            <Field label="Confirm password" id="confirmPassword" type={showCpw ? "text" : "password"} value={form.confirmPassword} onChange={setField("confirmPassword")} placeholder="Repeat password" error={errors.confirmPassword} icon={Lock} rightSlot={eyeBtn(showCpw, () => setShowCpw(p => !p))} />

            <p className="text-[10.5px] font-semibold uppercase tracking-wider mb-2 mt-1" style={{ color: "#334155", ...F }}>I want to join as</p>
            <div className="grid grid-cols-2 gap-2 mb-6">
              {[
                { value: "reader", icon: "◎", label: "Reader", hint: "Browse & collect", ic: ACC },
                { value: "writer", icon: "✒", label: "Writer", hint: "Publish & earn", ic: "#818CF8" },
              ].map(({ value, icon, label, hint, ic }) => {
                const active = form.role === value;
                return (
                  <button key={value} type="button" onClick={() => setRole(value)}
                    className="flex flex-col items-center gap-1 py-3 px-2 rounded-[10px] transition-all"
                    ...
                    <span style={{ fontSize: 16, color: active ? ic : "#334155" }}>{icon}</span>
                    <span className="text-[12px] font-semibold" style={{ color: active ? ACC : "#475569" }}>{label}</span>
                    <span className="text-[10px]" style={{ color: "#1E293B" }}>{hint}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="flex items-center gap-2.5 my-4">
            ...
            <span className="text-[10px] tracking-wider text-[#1E293B]">or continue with</span>
            <div className="flex-1 h-px bg-white/10" />
          </div>

          <button onClick={handleGoogle} className="w-full flex items-center justify-center gap-2 py-3 rounded-[10px] border border-white/10 hover:border-white/30 transition-all">
            ...
            >
              ...
              <div className="space-y-4">
                ...
                >
                  ...
                  <p className="text-sm text-gray-400">Browse, read and collect books</p>
                </button>
                ...
                >
                  ...
                  <p className="text-sm text-gray-400">Publish books and earn money</p>
                </button>
              </div>
            </motion.div>
          </div>
