...
function CustomTooltip({ active, payload, label, money }) {
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-[#1B1E28] px-4 py-3 shadow-xl">
      <p className="text-xs uppercase tracking-wider text-[#8B90A3] mb-1">{label}</p>
      {payload.map((p, i) => (
...
function StatCard({ title, value, isMoney, sub, icon: Icon, accent, delay, sparkData, sparkKey }) {
  const animated = useCountUp(isMoney ? Number(value) : value);
  const display_value = isMoney ? fmtMoney(animated) : Math.round(animated).toLocaleString();
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between">
        ...
        {sparkData ? <Sparkline data={sparkData} dataKey={sparkKey} color={accent} /> : null}
      </div>
      <p className={`${display.className} mt-5 text-4xl md:text-[2.75rem] leading-none text-[#F4F1EA]`}>
        {display_value}
      </p>
      <p className="mt-2 text-sm text-[#8B90A3]">{title}</p>
      {sub ? <p className="mt-1 text-xs" style={{ color: accent }}>{sub}</p> : null}
    </motion.div>
  );
}
      ...
      <main className="min-w-0 flex-1 px-5 py-8 md:px-10 md:py-12">
        ...
        {error && (
          <div className="mt-6 rounded-xl border border-[#E2836F]/30 bg-[#E2836F]/10 px-4 py-3 text-sm text-[#E2836F]">
            {error} — check that <code className="text-[#F4F1EA]">/api/v1/admin/analytics</code> is running on your backend.
          </div>
        )}

        {/* Stat cards */}
        <div className="mt-8 mb-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard title="Total Readers" value={analytics?.totalUsers ?? 0} icon={Users} accent={PALETTE.gold} delay={0} />
          <StatCard title="Writers" value={analytics?.totalWriters ?? 0} icon={Feather} accent={PALETTE.teal} delay={0.06} />
          <StatCard
            title="Ebooks Sold"
        ...
        <div className="mb-10 grid grid-cols-1 gap-5 lg:grid-cols-5">
          ...
          >
            <div className="mb-6 flex items-center justify-between">
              <div>
                <p className="mb-1 text-xs uppercase tracking-[0.2em] text-[#8B90A3]">Revenue Ledger</p>
                <p className="text-sm text-[#8B90A3]">Last six months</p>
              </div>
              <TrendingUp className="h-5 w-5 text-[#D4A657]" strokeWidth={1.8} />
            </div>
          ...
          >
            <div className="mb-6 flex items-center justify-between">
              <p className="text-xs uppercase tracking-[0.2em] text-[#8B90A3]">Genre Shelf</p>
            ...
            {genreDistribution.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={genreDistribution} layout="vertical" margin={{ left: 8, right: 16 }}>
                  <CartesianGrid strokeDasharray="3 6" stroke="#ffffff10" horizontal={false} />
                  <XAxis type="number" stroke="#8B90A3" fontSize={12} tickLine={false} axisLine={false} allowDecimals={false} />
                  <YAxis type="category" dataKey="genre" stroke="#8B90A3" fontSize={12} tickLine={false} axisLine={false} width={90} />
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="count" name="Titles" fill={PALETTE.teal} radius={[0, 6, 6, 0]} barSize={16} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </motion.div>
        ...
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-5">
          ...
          >
            <div className="mb-5 flex items-center justify-between">
              <p className="text-xs uppercase tracking-[0.2em] text-[#8B90A3]">Top Authors</p>
                  ...
                  {topWriters.map((w, i) => (
                    ...
                    >
                      <div className="flex min-w-0 items-center gap-3">
                        <span className={`${display.className} w-5 text-sm italic text-[#8B90A3]`}>
                          {String(i + 1).padStart(2, "0")}
                        </span>
                        <p className="truncate text-sm text-[#F4F1EA]">{w.writerName}</p>
                      </div>
                      <div className="ml-3 shrink-0 text-right">
                        <p className="text-sm font-medium text-[#D4A657]">{fmtMoney(w.revenue)}</p>
          ...
          >
            <div className="mb-5 flex items-center justify-between">
              <p className="text-xs uppercase tracking-[0.2em] text-[#8B90A3]">Recent Transactions</p>
                  ...
                  {recentTransactions.map((t) => (
                    ...
                    >
                      <div className="min-w-0 pr-3">
                        <p className="truncate text-sm text-[#F4F1EA]">{t.ebookTitle}</p>
                        <p className="truncate text-xs text-[#8B90A3]">{t.buyerEmail}</p>
                      </div>
                      <p className="shrink-0 text-sm font-medium text-[#F4F1EA]">{fmtMoney(t.amount)}</p>
                    </motion.div>
                  ))}
                </AnimatePresence>
              </div>
...
function EmptyChart({ label, compact }) {
  return (
    <div className={`flex items-center justify-center rounded-xl border border-dashed border-white/10 ${compact ? "h-24" : "h-[260px]"}`}>
