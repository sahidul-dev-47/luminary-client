...
import {
  BookPlus, Upload, X, CheckCircle2, AlertCircle,
...
const GENRES = [
  ...
  "Fantasy", "Horror", "Thriller", "Poetry",
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label
        ...
        style={{ color: error ? "#EF4444" : "#475569", fontFamily: "'Inter',sans-serif" }}
      >
      ...
      <AnimatePresence>
        ...
        {hint && !error && (
          <p className="text-[11px]" style={{ color: "#1E293B", fontFamily: "'Inter',sans-serif" }}>
            {hint}
          </p>
        )}
      </AnimatePresence>
...
export default function AddEbookFormPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const writerName  = session?.user?.name  || session?.session?.user?.name  || "Unknown Author";
      const writerEmail = session?.user?.email || session?.session?.user?.email || "unknown@fable.com";
      ...
      const resData = await createEbook({ 
        ...
        genre: form.genre,
        status: form.status === "published" ? "Available" : "Draft",
        coverImage,
      });

      if (resData && (resData.acknowledged || resData._id)) {
        setSuccess(true);
        setTimeout(() => router.push("/dashboard/writer"), 2000);
      } else {
      ...
      <div className="min-h-screen bg-[#07070E] flex items-center justify-center">
        ...
        >
          <motion.div
            ...
            style={{ background: "rgba(244,196,48,0.12)", border: "0.5px solid rgba(244,196,48,0.3)" }}
          >
            <CheckCircle2 size={40} style={{ color: ACCENT }} strokeWidth={1.5} />
          </motion.div>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "1.8rem", color: "#EDE9E0", fontWeight: 700 }}>
            Ebook Published!
          </h2>
          <p style={{ color: "#475569", fontFamily: "'Inter',sans-serif", fontSize: "14px" }}>
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 py-12">
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            <div
              ...
              style={{ background: `rgba(${ACCENT_RGB},0.12)`, border: `0.5px solid rgba(${ACCENT_RGB},0.25)` }}
            >
              <BookPlus size={18} style={{ color: ACCENT }} strokeWidth={1.8} />
            </div>
            <span
          ...
          >
            Add a new{" "}
            <em style={{ fontStyle: "italic", fontWeight: 400, color: ACCENT }}>ebook</em>
          </h1>
          <p style={{ color: "#334155", fontFamily: "'Inter',sans-serif", fontSize: "13.5px", marginTop: "6px" }}>
        ...
        >
          ...
          <Field label="Cover Image" required error={errors.image}>
            <div
              ...
              style={{
                ...
                background: isDragging ? `rgba(${ACCENT_RGB},0.05)` : "rgba(255,255,255,0.02)",
              ...
              {imagePreview ? (
                <div className="relative">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center gap-3 opacity-0 hover:opacity-100 transition-opacity duration-200"
                    ...
                    <button
                      ...
                      style={{ background: ACCENT, color: "#07070E", fontFamily: "'Inter',sans-serif" }}
                    >
                    ...
                    <button
                      ...
                      style={{ background: "rgba(239,68,68,0.15)", color: "#EF4444", border: "0.5px solid rgba(239,68,68,0.3)", fontFamily: "'Inter',sans-serif" }}
                    >
                ...
                <div className="flex flex-col items-center justify-center gap-3 py-10 px-4">
                  <div
                    ...
                    style={{ background: `rgba(${ACCENT_RGB},0.1)`, border: `0.5px solid rgba(${ACCENT_RGB},0.2)` }}
                  >
                    <ImageIcon size={22} style={{ color: ACCENT }} strokeWidth={1.5} />
                  </div>
                  <div className="text-center">
                    <p className="text-[13px] font-medium" style={{ color: "#94A3B8", fontFamily: "'Inter',sans-serif" }}>
                      Drag & drop or{" "}
                      <span style={{ color: ACCENT, textDecoration: "underline" }}>browse</span>
                    </p>
                    <p className="text-[11px] mt-1" style={{ color: "#1E293B", fontFamily: "'Inter',sans-serif" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <Field label="Genre" required error={errors.genre}>
              <div className="relative">
                ...
                <select
                  ...
                  style={{ ...inputBase, paddingLeft: "36px", appearance: "none", cursor: "pointer" }}
                  value={form.genre}
            ...
            {/* Price */}
            <Field label="Price (USD)" required error={errors.price} hint="Enter 0 for a free ebook">
              <div className="relative">
                <DollarSign
                  size={14} strokeWidth={1.8}
                  className="absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none"
          ...
          {/* ── Publish toggle ── */}
          <Field label="Visibility" hint="Unpublished ebooks are only visible to you.">
            <div className="flex gap-3">
              {[
                { val: "published",   label: "Publish now",   icon: Eye    },
                { val: "unpublished", label: "Save as draft", icon: EyeOff },
                ...
                <button
                  ...
                  style={{
                    background: form.status === val ? `rgba(${ACCENT_RGB},0.12)` : "rgba(255,255,255,0.03)",
                    border: `0.5px solid ${form.status === val ? `rgba(${ACCENT_RGB},0.35)` : "rgba(255,255,255,0.08)"}`,
                    color: form.status === val ? ACCENT : "#334155",
                    fontFamily: "'Inter',sans-serif",
                  }}
                >
          ...
          >
            {isLoading ? (
              <>
                ...
                {uploading ? "Uploading image…" : "Publishing ebook…"}
              </>
            ) : (
              <>
                <BookPlus size={16} strokeWidth={2.5} />
                {form.status === "published" ? "Publish ebook" : "Save as draft"}
              </>
            )}
          </button>

          {/* Bottom note */}
          <p className="text-center text-[11px]" style={{ color: "#1E293B", fontFamily: "'Inter',sans-serif" }}>
            Cover image is uploaded to imgBB · You can edit or delete this ebook anytime
          </p>
        </motion.form>
      </div>
    </div>
